class SCR_TaskGenerator
{
	...
	{
		// maybe also make one with {B9AEDBE988F20A1A}Prefabs/Structures/Infrastructure/Towers/AntennaR404_01/AntennaR404_01.et
		vector origin =  slot.GetOrigin();
	...
	void CreateKillEnemyTask(SCR_SiteSlotEntity slot, IEntity area)
	{
		...
		// Spawn ScenarioFramework area/layer/slot
		IEntity layerTaskDestroy = SCR_SpawnSetup.SpawnEntity(SCR_SpawnSetup.CreateZeroVector(origin), "{2008B4EE6C4D528E}Prefabs/ScenarioFramework/Components/LayerTaskKill.et", "CreateKillEnemyTask/SpawnLayerTaskKill", false);
		area.AddChild(layerTaskDestroy, -1);
		...
		layerTaskDestroy.AddChild(slotKill, -1);
		...
		IEntity character = IEntity.Cast(SCR_SpawnSetup.SpawnEntity(origin, characterResourceName, "CreateDestroyRadarTask/SpawnEntity"));
